#!/usr/bin/env python3
"""
POS Terminal — fake payment terminal for a Raspberry Pi Zero WH.

Drives an SSD1306 OLED (I2C) and an RC522 RFID reader (SPI). Tap any card
and it plays a convincing "contacting bank" animation before approving the
transaction. No real payment happens — it's pure theatre.
"""
import time
...
# 0x3C is the default I2C address for these SSD1306/SSD1315 boards
...
try:
    ...
    # Fall back to the bundled bitmap font if DejaVu isn't installed
...
def main():
    ...
    try:
        while True:
            ...
            # Non-blocking presence check — SimpleMFRC522.read() would block
            # the loop, so we poll the underlying reader for a card in the field
